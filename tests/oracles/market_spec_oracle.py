...
def _range_position_spec(close: float, lows: np.ndarray, highs: np.ndarray, eps: float = 1e-12) -> float:
...
def _build_htf_context_spec_frame(
    df: pd.DataFrame,
    *,
    close: np.ndarray,
    high: np.ndarray,
    ...
    if "day_id" in df.columns and n:
        ...
        if not daily_valid.empty:
            ...
            d_slope5 = _rolling_slope_spec(d_log_close, 5)
            d_slope10 = _rolling_slope_spec(d_log_close, 10)
            d_range_pct = _rolling_percentile_causal_spec(d_range, window=240, min_periods=10)
            ...
            for i in range(n):
                ...
                if last >= 4:
                    out[i, col["daily_close_pos_in_range_5"]] = _range_position_spec(
                        last_close,
                        d_low[last - 4:last + 1],
                ...
                if last >= 9:
                    out[i, col["daily_close_pos_in_range_10"]] = _range_position_spec(
                        last_close,
                        d_low[last - 9:last + 1],
    ...
    if completed_h1 > 0:
        ...
        h1_slope6 = _rolling_slope_spec(h1_log_close, 6)
        h1_slope12 = _rolling_slope_spec(h1_log_close, 12)
        h1_range_pct = _rolling_percentile_causal_spec(
        ...
        for i in range(n):
            ...
            if h >= 5:
                out[i, col["h1_close_pos_in_range_6"]] = _range_position_spec(
                    last_close,
                    h1_low[h - 5:h + 1],
                    h1_high[h - 5:h + 1],
                    eps=eps,
            ...
            if h >= 11:
                out[i, col["h1_close_pos_in_range_12"]] = _range_position_spec(
                    last_close,
                    h1_low[h - 11:h + 1],
                    h1_high[h - 11:h + 1],
                    eps=eps,
...
def _add_obs_features_spec(df: pd.DataFrame) -> pd.DataFrame:
    eps = 1e-12
    ...
    htf_context = _build_htf_context_spec_frame(df, close=C, high=H, low=L, valid=valid, mask=mask_np, eps=eps)
